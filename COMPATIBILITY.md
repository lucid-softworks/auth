...
| PostgreSQL | Supported | Core migrations include normalized email uniqueness, username-optional credential accounts, and Better Auth 1.7 issuer-qualified OAuth accounts with encrypted-token columns. Optional plugins contribute their own tables. Lifecycle, same-subject/different-issuer identity, atomic provider-token rotation and final-account unlink protection, concurrent case-variant signup, plugin migration idempotence, atomic API-key claims, and atomic two-factor replay/backup-code/lockout writes run in the live contract. Broader schema generation is [#68](https://github.com/lucid-softworks/auth/issues/68). |
