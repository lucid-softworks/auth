...
| Dashboard and audit logs | Partial | `DashPlugin` matches 61 hosted/browser `/dash/*` routes and four local-session event-query routes from `@better-auth/infra@0.4.3`: the 26-route core family; all 35 organization, member, team, invitation, and managed two-factor routes; managed JWT/JTI policy; opt-in activity tracking; five raw adapter actions; the exact 39-value event catalog and 35-event projection; one-shot remote tracking; remote event transformation/filtering; local membership/role authorization; and the two-action `dashClient()` declaration. Organization and two-factor state reuses the native plugins and stores; public invitation completion preserves the managed-token/local-session boundary and exact remote egress. Directory, SSO, and SCIM management remains tracked by [#92](https://github.com/lucid-softworks/auth/issues/92), so the overall Dashboard family remains Partial. See [Dash core routes 0.4.3](#dash-core-routes-043), [Dash organization and two-factor routes 0.4.3](#dash-organization-and-two-factor-routes-043), [Dash event projection 0.4.3](#dash-event-projection-043), [Dash event queries 0.4.3](#dash-event-queries-043), and [Dash substrate 0.4.3](#dash-substrate-043). |
