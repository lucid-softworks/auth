...
| --- | --- |
| Sessions | Opaque list/revoke tokens, database and secondary-backed sessions, stateless cookie sessions, and all three cookie-cache strategies pass Rust and official 1.7.1 decoding contracts. Sliding `updateAge`/deferred refresh remains [#81](https://github.com/lucid-softworks/auth/issues/81). |
...
| Custom and secondary storage | Typed database hooks/schema fields plus secondary-backed sessions and rate limits are supported. Secondary verification values remain [#79](https://github.com/lucid-softworks/auth/issues/79). |
...
| --- | --- | --- |
| `getSession`, `useSession` (`GET /get-session`) | Supported | Database, secondary, stateless, cache bypass/version/expiry/token binding, compact, HS256 JWT, and A256CBC-HS512 JWE profiles; [#58](https://github.com/lucid-softworks/auth/issues/58). Sliding DB refresh remains [#81](https://github.com/lucid-softworks/auth/issues/81). |
| `signOut` (`POST /sign-out`) | Supported | Revokes stateful/secondary sessions and clears primary/cache cookies. Pure stateless sessions use cache expiry/version invalidation, matching Better Auth; [#58](https://github.com/lucid-softworks/auth/issues/58). |
...
| `listAccounts`, `accountInfo` | Supported | Exact account identifiers, issuer/subject identity, parsed scopes, provider user/profile data, owner-bound durable account-ID selection, and automatic near-expiry refresh pass the official client. Optional `account_data` cookie storage remains [#80](https://github.com/lucid-softworks/auth/issues/80); [#15](https://github.com/lucid-softworks/auth/issues/15). |
...
| Secondary session/rate-limit storage and stateless sessions | Supported | Exact secondary primitive, authoritative session routing, optional DB mirror/preservation, stateless compact/JWT/JWE cache profiles, and default secondary rate limiting; [#58](https://github.com/lucid-softworks/auth/issues/58). Secondary verification values remain [#79](https://github.com/lucid-softworks/auth/issues/79). |
