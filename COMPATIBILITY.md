...
| Secondary session/rate-limit storage and stateless sessions | Supported | Exact secondary primitive, authoritative session routing, update-age refresh with active-reference TTL renewal, optional DB mirror/preservation, stateless compact/JWT/JWE cache profiles, and default secondary rate limiting; [#58](https://github.com/lucid-softworks/auth/issues/58) and [#81](https://github.com/lucid-softworks/auth/issues/81). Secondary verification values remain [#79](https://github.com/lucid-softworks/auth/issues/79). |
