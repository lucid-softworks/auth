...
| Two-factor responses | Supported through optional `TwoFactorPlugin`: the official 1.7.2 discriminated enable response, all eight browser-client endpoints, server-only TOTP generation and backup-code viewing, challenge/trust cookies, and encrypted factor storage are covered by [#20](https://github.com/lucid-softworks/auth/issues/20). |
| Passwordless cleanup | Magic Link, Email OTP, and Phone Number implement Better Auth 1.7.2's atomic one-time token lifecycle. Phone Number keeps password sign-in separate from passwordless OTP verification/signup; [#22](https://github.com/lucid-softworks/auth/issues/22), [#23](https://github.com/lucid-softworks/auth/issues/23), and [#24](https://github.com/lucid-softworks/auth/issues/24). |
| Google One Tap | The optional native plugin verifies Google's RS256 ID token against Google's JWKS, issuer, audience, expiry, and one-hour maximum age; applies Google hosted-domain, signup, linking, email-verification, anonymous-upgrade, and session policy; and exposes Better Auth's exact browser callback contract. The official 1.7.2 GIS/FedCM client flow is covered by [#25](https://github.com/lucid-softworks/auth/issues/25). |
...
| Error contracts | The 1.7.2 official client suite asserts structured status/code handling, including enumeration-safe `401 INVALID_EMAIL_OR_PASSWORD` and `401 INVALID_USERNAME_OR_PASSWORD`; each new endpoint must add its own error regressions before becoming Supported. |
| Release regressions | Permanent re-bans clear a previous temporary expiry, D1 programmatic migrations introspect existing indexes, and CIMD authorization accepts the intersection of client and provider grant support. Placeholder social emails, signed-cache schema warnings, safe relative URLs/query fragments, null-origin Fetch Metadata requests, and disabled MyISAM indexes have dedicated regressions; [#127](https://github.com/lucid-softworks/auth/issues/127), [#128](https://github.com/lucid-softworks/auth/issues/128), [#130](https://github.com/lucid-softworks/auth/issues/130), and [#131](https://github.com/lucid-softworks/auth/issues/131). |
| TypeScript-only release changes | Client generic assignability, AsyncLocalStorage behavior, JavaScript test hashers, and the JavaScript logger API do not cross the native process boundary. Native equivalents are tested through Rust types, request context, password hashing, and `tracing`; they are not claims of TypeScript runtime compatibility. |
...
Database IDs match Better Auth 1.7.2's `advanced.database.generateId` contract
...
transforms. This is upstream 1.7.2 behavior, not a compatibility alias.
...
| `verifyPassword` (`POST /verify-password`) | Supported | Session-bound credential verification with the 1.7.2 status/error body; [#9](https://github.com/lucid-softworks/auth/issues/9). |
...
| `updateSession` (`POST /update-session`) | Supported | Typed configured additional fields persist in memory/SQLite/PostgreSQL, protected core fields cannot be overwritten, returned-false fields are omitted, and the updated session/cookie shape passes the official client; [#12](https://github.com/lucid-softworks/auth/issues/12). |
...
| Two-Factor Authentication | Supported | Optional `TwoFactorPlugin`; all official `twoFactorClient` methods, exact 1.7.2 response fields and error codes, TOTP and delivered OTP, encrypted TOTP secrets/backup codes, one-time TOTP counters, atomic backup-code use, challenge attempt budgets, account lockout, rotating expiring trusted devices, server-only TOTP generation/backup-code viewing, and memory/SQLite/PostgreSQL plugin stores; [#20](https://github.com/lucid-softworks/auth/issues/20). Role-driven step-up is a separate optional lucid extension. |
...
| Sign In With Ethereum | Supported | Optional `SiwePlugin`; both nonce aliases and `verify` pass the official `siweClient` against the native server. The 8–250 ASCII-alphanumeric nonce, 15-minute single-use storage, Better Auth 1.7.2 line parser/checksum behavior and pinned JavaScript-compatible time forms, consume-before-verification ordering, callback payload, anonymous/email user resolution, ENS profile lookup, CAIP-122 projection, same-wallet cross-chain linking, `local:siwe` accounts, normal sessions, exact validation/errors, and memory/SQLite/PostgreSQL atomic persistence are covered by [#26](https://github.com/lucid-softworks/auth/issues/26). |
| Generic OAuth | Supported | Optional `GenericOAuthPlugin`; providers are prepended to the ordinary social registry and use only `signIn.social` plus `/callback/:id`. Exact 1.7.2 discovery precedence/failure rules, OAuth/OIDC classification, PKCE and nonce defaults, JOSE algorithm allowlisting including ES512, all four token auth methods, custom token/user/profile/identity callbacks, static or request-aware refresh params, IDP-initiated restart, provider logout, signup/profile controls, duplicate/shadow ordering, database state or Better Auth-compatible XChaCha20-Poly1305 cookie state, both plugin errors, and all ten bundled helper presets are covered by Rust and official-client contracts; [#27](https://github.com/lucid-softworks/auth/issues/27). |
| Multi Session | Supported | Optional `MultiSessionPlugin`; all three official `multiSessionClient` methods, signed per-session selector cookies, same-user replacement, configurable quota boundaries, unique active-device listing, activation without a current session, authenticated revocation with current-session replacement, stateful cookie-cache authorization, and verified-selector cleanup on sign-out are covered against Better Auth 1.7.2 in memory, PostgreSQL, and official-client contracts; [#28](https://github.com/lucid-softworks/auth/issues/28). |
| Last Login Method | Supported | Optional `LastLoginMethodPlugin`; exact email/callback/SIWE/passkey/magic-link default resolution, custom fallback and empty suppression, unsigned URI-encoded session-cookie-scoped tracking, async cookie consent, configurable name and Better Call max-age boundaries, optional input-disabled user schema storage, and the three synchronous `lastLoginMethodClient` actions are covered against Better Auth 1.7.2; [#29](https://github.com/lucid-softworks/auth/issues/29). |
| OAuth Popup | Supported | Optional `OAuthPopupPlugin`; exact `GET /oauth-popup/start`, state strategy, signed `oauth_popup` marker, provider redirect, callback completion document/CSP, session-token relay, popup validation and failure ordering, per-plugin cookie override, and official `oauthPopupClient` browser/storage/fetch behavior are covered against Better Auth 1.7.2. Top-level popup sign-in works independently; cross-origin embedded clients also require Bearer support tracked in [#34](https://github.com/lucid-softworks/auth/issues/34). The marker is deliberately separate from OAuth state and one fixed cookie can be overwritten by concurrent popup starts, matching upstream; [#70](https://github.com/lucid-softworks/auth/issues/70). |

## Authorization and management plugins

| Plugin | Status | Tracking and limitations |
| --- | --- | --- |
| Admin | Supported | Optional `AdminPlugin`; all 15 official `adminClient` methods pass against Better Auth 1.7.2. This includes passwordless creation, flattened additional user fields, get/update, permission checks, safe filtering/sorting/pagination, multiple roles, configurable access-control statements and admin IDs, ban defaults/messages, bounded impersonation, and exact response/error shapes. Its routes and user fields are absent when disabled; [#18](https://github.com/lucid-softworks/auth/issues/18), [#75](https://github.com/lucid-softworks/auth/issues/75). |
...
| SSO | Supported | Optional `SsoPlugin` matches `@better-auth/sso@1.7.2`: official provider management and `ssoClient`, OIDC discovery/manual endpoints/PKCE/private-key JWT, verified identity and transactional resolution, SAML metadata/ACS/SLO/signature/encryption policy/replay controls, domain verification, schema/additional fields, provisioning, and Organization assignment; [Enterprise SSO 1.7.2](#enterprise-sso-172), [#31](https://github.com/lucid-softworks/auth/issues/31). |
| SCIM | Supported | Native server-only `@better-auth/scim@1.7.2` Users/Groups provisioning, public discovery, static/application/managed bearer resolution, identity lifecycle, role projection, managed credentials/events, and resumable connection retirement; [SCIM 1.7.2](#scim-172), [#32](https://github.com/lucid-softworks/auth/issues/32). |

## Enterprise SSO 1.7.2

`SsoPlugin` pins the server package and official `ssoClient` to
`@better-auth/sso@1.7.2`. The client boundary contains `signIn.sso`, provider
...
`ScimPlugin` targets the immutable `@better-auth/scim@1.7.2` server package.
...
testing, and a single service process. With the `postgres` feature, durable
deployments construct `PostgresAgentAuthStore` from the schema-bound
`PostgresStore`. With the `sqlite` feature, pass a clone of the schema-bound
`SqliteStore` directly to `AgentAuthPlugin::new`. Adapter-owned schema evolution
includes the four remappable models `agentHost`, `agent`,
`agentCapabilityGrant`, and `approvalRequest`. Agent Auth does not contribute
independent static SQL.
...
The memory, PostgreSQL, and SQLite stores implement the same atomic
registration, approval, grant, enrollment, rotation, reactivation,
account-switch, cleanup, and cascading-revocation transitions. PostgreSQL uses
transactions and advisory/row locking for cross-instance state changes. SQLite
uses immediate transactions to serialize state transitions across pooled
connections. Schema remapping applies to all four plugin models and their
relationships; it is not a second wire contract.
...
`Dub OAuth is not configured`. With `oauth`, Better Auth 1.7.2's
...
Conformance pins Better Auth `1.7.2`, Better Call `1.4.0`, their Zod `4.4.3`,
...
### Dash core routes 0.4.3

`DashPlugin` owns exactly 26 routes under the configured auth base path: ten
GET and sixteen POST routes. The family covers `/dash/config`,
`/dash/validate`, `/dash/execute-adapter`, user list/export/create/read/update/
delete and bulk deletion, organization lookup, account unlinking, password and
session operations, impersonation, user statistics/graph/retention,
ban/unban and bulk banning, plus verification and password-reset email actions.
The four local-session event routes are documented separately below and do not
share this hosted authorization boundary.

Every route requires the hosted Dash JWT. Ordinary routes enforce token age,
signature, API-key hash, expiry, route claims, and JTI for tokens at least 30
seconds old. `/dash/validate` deliberately skips only JTI and route-claim
validation. A local Better Auth session is not a fallback. `/dash/config`
returns the pinned redacted capability snapshot and never returns the Dash API
key, Better Auth secret, OAuth client secrets, password hashes, session tokens,
or account tokens.

User export is streamed as `application/x-ndjson` in batches of 10,000, closes
after a five-minute stalled consumer, and returns the pinned failed-dependency
response when there is nothing to export. The raw adapter endpoint accepts only
`findOne`, `findMany`, `create`, `update`, and `count`, with the pinned where,
selection, sorting, join, limit, and offset vocabulary. It is an authenticated
database administration surface, not a general remote SDK proxy.

Activity tracking is disabled by default. Enabling it contributes the optional
`lastActiveAt` user field, records session creation, and refreshes
activity after authenticated non-GET requests only when the configured interval
has elapsed. The three email actions delegate the application's existing Better
Auth verification/reset callbacks and their availability; installing Dash does
not create an email transport.

The Dash API origin receives the configured API key, hosted JWT/JTI checks, and
JWKS requests. The core routes expose administrative user and authentication
data to callers holding a valid hosted Dash token. Treat that token and origin
...
### Dash organization and two-factor routes 0.4.3

`DashPlugin` also owns the pinned 35-route family: eleven GET and twenty-four
POST routes for organization list/export/read/create/update/deletion, member
management, teams and team membership, invitations, existence checks, and
managed TOTP/backup-code administration. List routes preserve the artifact's
empty results when the corresponding native plugin is absent or teams are
disabled; mutation routes preserve its explicit plugin/feature errors.

Administrative routes require the hosted Dash JWT and their exact per-route
`userId`, `organizationId`, `organizationIds`, `invitationId`, or `invitedBy`
claims. They reuse the installed `OrganizationPlugin` and `TwoFactorPlugin`
stores, limits, configured roles, database ID policy, and lifecycle hooks.
Managed TOTP setup stores encrypted secrets and backup codes, exposes plaintext
only during initial enable or code regeneration, and permanently forbids
viewing existing backup codes.

Invitation accept, completion, handoff, and social completion are browser
flows rather than hosted-administration JWT routes. Verification, expiration,
acceptance, and one-time handoff redemption call the exact
`/api/internal/invitations/*` Dash API operations with the configured API key.
The four pinned platform auth modes (`auth`, `credential_setup`,
`create_with_session`, and `create_no_session`) retain their distinct redirect,
passwordless, session-creation, and session-deletion behavior. Redirects are
limited to the configured auth or trusted HTTP(S) origins. Invitation email,
user identifiers, handoff material, and acceptance state leave the process for
the configured API origin; local credentials, TOTP secrets, backup codes, and
session tokens do not.

### Dash directory, SSO, and SCIM control plane 0.4.3

The exact 18-function control plane occupies 17 unique paths: seven SSO
provider/domain routes, four legacy-directory routes, and seven managed
directory-lifecycle routes. The shared `/dash/organization/:id/directories`
path intentionally supports both GET and POST. Native SCIM 1.7 exposes managed
mode only; it does not pretend to implement the older provider-ownership token
surface. Managed mode is additionally gated by
`managed_directory_sync.enabled` and contributes exactly the two pinned
directory catalog/provenance models.

Managed authorization binds `purpose`, organization, actor, and optional setup
operation claims. Setup claims may create or recover their exact connection but
cannot list, rotate, revoke, decommission, or unpair it. One-time SCIM bearer
tokens are returned only by create/recovery and rotation with no-store,
no-cache, and no-referrer headers; list/detail/config responses expose only
safe credential metadata. Paired SSO providers enforce exact organization,
protocol, external-ID source, active-provider uniqueness, signed SAML
assertions, authentication-boundary mutation guards, and SCIM-owned identity
resolution. SCIM user activation/deactivation projects organization membership
with created-versus-observed provenance so pre-existing memberships are never
deleted.

These routes send organization, provider, domain, directory, actor, user, and
membership identifiers through the configured Better Auth Infrastructure and
identity-provider boundaries. They can receive one-time SCIM credentials.
Hosted JWTs, API keys, OIDC client secrets, SAML private keys, and plaintext
SCIM bearer tokens are never logged or persisted outside the native encrypted
or digest-only stores. Configure only trusted origins and treat every returned
one-time credential as a server-side secret.
...
`/events/list`, `/events/audit-logs`, `/events/all-audit-logs`, and
`/events/types`. They require the configured Events API and a normal local
Better Auth session. User queries reject another `userId`; organization queries
require local membership; all-activity queries require exact `owner` or
`admin` roles and scope unqualified/user queries to the actor's elevated
organizations. Member access uses the configured organization store, including
schema remapping and memory, SQLite, PostgreSQL, or custom backends.

Paging, local event/identifier filters, filtered-total behavior, remote totals,
date conversion, location omission, error messages, and the exact 25 user plus
14 organization constants match the pinned artifact. The official
`@better-auth/infra/client` declaration has id `dash`, a single `dash`
namespace, only `getAuditLogs` and `getAllAuditLogs`, and GET `pathMethods` only
for those two paths. Its user resolver uses explicit `userId`, the configured
resolver, `user.id`, then `session.user.id`; the all-activity action does not
invoke it.

The API origin receives the API key plus transmitted user/organization/filter
identifiers and returns provider-owned audit records that can include email,
account, event-data, IP, city, country, and country-code PII. Lucid-auth adds no
local audit-event table, migration, history map, drain/replay endpoint, retry,
or retention policy. This surface is separate from the optional native
`AuditPlugin` and from hosted `/dash/**` management authorization.
...
Core instrumentation matches Better Auth 1.7.2 through the `better-auth`
scope at version `1.7.2`. Dispatch emits `{METHOD} {route}` with child
...
| Local SQLite | Supported | Optional native SQLx `SqliteStore` targets Better Auth and `@better-auth/kysely-adapter` `1.7.2`. It accepts a supplied pool, URL, or caller-built connection options; supports file and one-connection `:memory:` databases; uses the resolved model/field/ID catalog for core and official-plugin rows; provides bound CRUD/predicates/sort/select/count plus atomic consume/increment and real transactions; and derives pinned additive ordinary-table migrations with exact drift, safety, index, and array-warning boundaries. The host owns foreign keys, WAL, synchronous mode, busy timeout, shared cache, pooling, retries, checkpoints, and vacuum. Standard upstream Kysely introspection includes views, while the pinned Node/Bun dialects and this native backend enumerate ordinary tables only; [#61](https://github.com/lucid-softworks/auth/issues/61). |
| Cloudflare D1 | Supported | Optional native `D1Store` and `WorkersD1Database` target Better Auth and `@better-auth/kysely-adapter` `1.7.2` without enabling SQLx. Every configured core or supported-plugin model uses exact physical remaps and SQLite value types through bound `prepare().bind().all()` calls. Results preserve `results`, optional `meta.changes`, and optional `meta.last_row_id`; streaming and interactive transactions fail explicitly; consume and increment are single statements; introspection includes tables/views while excluding `sqlite_%`, `_cf_%`, and Kysely internals; only the closed `pragma_table_info` set is batched; additive migrations execute sequentially and preserve SQLite drift, unsafe-change, and index-conflict boundaries. SCIM is intentionally unavailable because D1 lacks the required native interactive transactions; [#32](https://github.com/lucid-softworks/auth/issues/32), [#96](https://github.com/lucid-softworks/auth/issues/96). |
| MySQL | Supported | Optional native SQLx `MySqlStore` targets Better Auth and `@better-auth/kysely-adapter` `1.7.2`. It binds the resolved schema and ID strategies, preserves MySQL boolean/date/JSON/array conversions, provides bound CRUD/predicates/sort/select/count plus transactional consume/increment and typed core/plugin stores, and derives table-scoped additive `information_schema` migrations with MySQL returning emulation and index safety. Startup requires SQLx's matched-row capability and `+00:00` sessions. Project CI uses the exact `mysql:8.4` fixture and pinned `mysql2@3.22.5` oracle without claiming a broader MySQL or MariaDB range; [#62](https://github.com/lucid-softworks/auth/issues/62). |
| MongoDB | Supported | Optional native official-driver `MongoStore` targets `@better-auth/mongo-adapter` `1.7.2`. It maps `id` to `_id`, preserves native BSON arrays/documents/dates, uses ObjectId/default, BSON UUID, database-deferred, and opaque callback ID rules, rejects serial IDs, implements aggregation predicates/projection/sort/pagination/joins plus atomic consume/increment, and provides typed core/plugin stores. Table-level indexes are lazy writes-only, deduplicated after success, retryable after failure, and skipped for `disableMigrations`; there is no collection migration ledger or topology preflight. A supplied client enables transactions unless explicitly disabled for standalone deployments. CI uses exact `mongo:8.2` standalone/replica fixtures and the pinned `mongodb@7.1.0` JavaScript oracle without claiming a broader server range; [#63](https://github.com/lucid-softworks/auth/issues/63). |
| Microsoft SQL Server | Supported | Optional native Tiberius/BB8 `MssqlStore` targets Better Auth and `@better-auth/kysely-adapter` `1.7.2` with Kysely `0.29.5`. It preserves MSSQL text/boolean/date/array conversions, string and serial ID/reference rules, resolved remapping, predicates, `TOP`/`OFFSET ... FETCH`, left-join grouping, `OUTPUT` mutation returns, atomic consume/increment, typed core/plugin stores, and explicitly enabled wrapper transactions. Its active-schema `sys`-catalog planner is additive, sequential, index-budget aware, and non-destructive. CI runs native live contracts and the pinned Tedious oracle against SQL Server 2022 without claiming a broader SQL Server or Azure SQL range; [setup, migration, and security guide](docs/mssql.md), [#64](https://github.com/lucid-softworks/auth/issues/64). |
...
| Framework and production installation guides | Supported | Runnable memory/SQLite/MySQL/PostgreSQL Axum examples, native Cloudflare D1 binding and migration configuration, explicit Cargo/client pins, React/Vue/Svelte/Solid/vanilla and SSR boundaries, plus proxy/TLS/cookie/CORS/migration guidance; [#65](https://github.com/lucid-softworks/auth/issues/65). |
