...
| Official JavaScript client conformance | Supported | CI drives the pinned `1.7.1` vanilla email/password, email-verification, password-reset, social OAuth, and linked-account/token clients plus username, anonymous, email OTP, admin, every passkey, API-key, organization, team, and dynamic-role client method, two-factor, and a native test-client plugin against an ephemeral server. Passkey registration/authentication use real signatures from an in-process virtual authenticator; [#2](https://github.com/lucid-softworks/auth/issues/2), [#14](https://github.com/lucid-softworks/auth/issues/14), [#15](https://github.com/lucid-softworks/auth/issues/15), [#23](https://github.com/lucid-softworks/auth/issues/23), and [#30](https://github.com/lucid-softworks/auth/issues/30). |
...
| Passwordless cleanup | Magic Link and Email OTP implement the 1.7 mailbox-proven account/session cleanup and atomic token lifecycle. Phone OTP remains [#24](https://github.com/lucid-softworks/auth/issues/24). |
...
| `getAccessToken`, `refreshToken` | Supported | Access, refresh, and ID tokens are encrypted at rest; responses decrypt only for the owning session. Strict account-ID or explicit `useAccountCookie: true` selection, owner binding, near-expiry refresh, cookie refresh, scope preservation, and atomic concurrent rotation pass the official client; [#15](https://github.com/lucid-softworks/auth/issues/15) and [#80](https://github.com/lucid-softworks/auth/issues/80). |
