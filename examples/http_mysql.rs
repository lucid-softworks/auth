...
use lucid_auth::{
    ...
    mysql::{MySqlAdapterConfig, MySqlStore},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let store = Arc::new(MySqlStore::connect(&database_url, MySqlAdapterConfig::default()).await?);
    ...
    println!("lucid-auth MySQL server listening on {public_url}");
    axum::serve(
        listener,
