...
pub(super) async fn all_fields_and_unique_codes(
    ...
    let mut expected = code("round-trip", Some(owner.into()));
    ...
    let stored = insert(store, auth_store, expected.clone()).await?;
    expected.id = stored.id.clone();
    assert!(expected.id.bytes().all(|byte| byte.is_ascii_digit()));
    ...
    assert_eq!(
        store
            .create_device_code(create(duplicate_device), auth_store)
            .await?,
    ...
    assert_eq!(
        store
            .create_device_code(create(duplicate_user), auth_store)
            .await?,
        DeviceCodeCreateOutcome::UniqueConflict
    );
