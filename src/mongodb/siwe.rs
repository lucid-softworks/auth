use super::{MongoComparisonMode, MongoFilter, MongoStore, codec, oauth, query::execute};
use crate::{
...
async fn create_identity(
    ...
    let outcome = if let Some(owner) = address_owner {
        ...
        let mut email = MongoFilter::equal("email", json!(user.email));
        email.mode = MongoComparisonMode::Insensitive;
...
async fn owner_tx(
    ...
    let user = execute::find_one(
        ...
        schema,
        "user",
        &[MongoFilter::equal("id", json!(wallet.user_id))],
        &[],
    )
    .await?
...
fn wallet_filters(address: &str, chain_id: Option<f64>) -> Vec<MongoFilter> {
    let mut filters = vec![MongoFilter::equal("address", json!(address))];
    if let Some(chain_id) = chain_id {
        filters.push(MongoFilter::equal("chainId", json!(chain_id)));
    }
    filters
}
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid MongoDB walletAddress row: {field}"))
}
