use super::{MongoFilter, MongoFilterOperator, MongoStore, query::execute};
use crate::{
...
#[async_trait]
impl SecurityStore for MongoStore {
...
async fn consume(
    ...
    let mut expired = MongoFilter::equal("lastRequest", json!(now_ms.saturating_sub(prune_ms)));
    expired.operator = MongoFilterOperator::Lt;
    execute::delete_many(connection, schema, "rateLimit", &[expired]).await?;
    let current = execute::find_one_for_update(
        connection,
        schema,
        "rateLimit",
        &[MongoFilter::equal("key", json!(key))],
        &[],
    )
    .await?;
...
async fn update(
    ...
    .await?
    .ok_or_else(|| AuthError::Storage("MongoDB rate-limit row disappeared".into()))?;
    Ok(())
}
...
fn integer(record: &Map<String, Value>, field: &str) -> Result<i64, AuthError> {
    record
        ...
        .ok_or_else(|| AuthError::Storage(format!("invalid MongoDB rateLimit row: {field}")))
}
