use super::{MongoFilter, MongoStore, query::execute};
...
#[async_trait]
impl TwoFactorStore for MongoStore {
...
async fn record_failure(
    ...
    let next = updated
        ...
        .unwrap_or(0);
    let locked = next >= u64::from(max_attempts);
    if locked {
        execute::update_one(
            &mut transaction,
            schema,
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid MongoDB twoFactor row: {field}"))
}
