use super::{MongoComparisonMode, MongoFilter, MongoStore, codec, oauth, passkey, session};
mod write;
use crate::{
...
impl AuthStore for MongoStore {
    ...
    async fn find_user_by_email(&self, email: &str) -> Result<Option<AuthUser>, AuthError> {
        let mut filter = MongoFilter::equal("email", json!(email));
    ...
    async fn update_user_profile(
        ...
        write::update_user(self, &[MongoFilter::equal("id", json!(id))], values).await
    }
    ...
    async fn update_user_email(
        ...
        let mut email = MongoFilter::equal("email", json!(expected_email));
        email.mode = MongoComparisonMode::Insensitive;
        write::update_user(
            self,
            &[MongoFilter::equal("id", json!(id)), email],
            Map::from_iter([
    ...
    async fn update_password_hash(&self, user_id: &str, hash: String) -> Result<(), AuthError> {
        ...
        oauth::update(
            self,
            &account,
            &[MongoFilter::equal("id", json!(account.id))],
        )
        .await?
        .ok_or(AuthError::CredentialAccountNotFound)?;
...
pub(super) async fn find(
    ...
    find_filtered(store, &[MongoFilter::equal(field, json!(value))]).await
}
