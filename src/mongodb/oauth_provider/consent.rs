...
use crate::{
    ...
    mongodb::{
        MongoFilter, MongoFindOptions, MongoSort, MongoSortDirection, MongoStore,
        query::execute,
    },
};
...
#[async_trait]
impl OAuthProviderConsentStore for MongoStore {
    ...
    async fn find_oauth_consent_for_grant(
        ...
        find(
            ...
            &[
                ...
                eq("userId", user_id),
                MongoFilter::equal("referenceId", json!(reference_id)),
            ],
        )
        .await
    }
    ...
    async fn upsert_oauth_consent(
        ...
        let filters = [
            ...
            MongoFilter::equal("userId", json!(consent.user_id)),
            MongoFilter::equal("referenceId", json!(consent.reference_id)),
        ];
            ...
            execute::find_one(&mut transaction, schema, "oauthConsent", &filters, &[]).await?
        {
            let existing_id = existing
                ...
                .ok_or_else(|| AuthError::Storage("invalid MongoDB oauthConsent row: id".into()))?;
