...
use super::super::{MongoFilter, MongoStore, codec, oauth};
    ...
    match value {
        ...
        DatabaseWrite::Update(mut user) => {
            ...
            let stored = super::super::query::execute::update_one(
                ...
                schema,
                "user",
                &[MongoFilter::equal("id", json!(user.id))],
                values,
            )
            .await?
...
async fn credential_account(
    ...
    super::super::query::execute::find_one(
        ...
        &[
            ...
            MongoFilter::equal("providerId", json!("credential")),
        ],
        &[],
    )
    .await?
...
pub(super) async fn promote(
    ...
    let schema = store.physical_schema()?;
    let mut transaction = store.begin().await?;
    let filter = [MongoFilter::equal("userId", json!(id))];
...
pub(super) async fn set_password(
    ...
    if account.id.is_empty() {
        ...
        let id = account.id.clone();
        oauth::update(store, &account, &[MongoFilter::equal("id", json!(id))])
            .await?
            .ok_or(AuthError::CredentialAccountNotFound)?;
    }
...
pub(super) fn require_field(
    model: &super::super::schema::MongoModel<'_>,
    field: &str,
) -> Result<(), AuthError> {
