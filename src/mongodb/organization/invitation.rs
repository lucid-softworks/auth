...
use crate::{
    ...
    mongodb::{
        MongoComparisonMode, MongoFilter, MongoFindOptions, MongoSort, MongoSortDirection,
        MongoStore, query::execute,
    },
};
...
#[async_trait]
impl OrganizationInvitationStore for MongoStore {
    ...
    async fn resend_invitation(
        ...
        email_filter.mode = MongoComparisonMode::Insensitive;
        let filters = [
        ...
        let id = latest
            ...
            .ok_or_else(|| AuthError::Storage("invalid MongoDB invitation row: id".into()))?;
        self.update_record(
            "invitation",
            &[eq("id", id)],
...
fn pending_filters(organization_id: &str, email: Option<&str>) -> Vec<MongoFilter> {
    let mut filters = vec![
        eq("organizationId", organization_id),
        eq("status", "pending"),
    ];
