...
fn latest_options() -> MongoFindOptions {
    MongoFindOptions {
        sort: Some(MongoSort {
...
fn eq(field: &str, value: impl serde::Serialize) -> MongoFilter {
    MongoFilter::equal(field, json!(value))
}

fn less_than(field: &str, value: serde_json::Value) -> MongoFilter {
    MongoFilter {
        field: field.into(),
        value,
        operator: super::MongoFilterOperator::Lt,
        connector: super::MongoFilterConnector::And,
        mode: super::MongoComparisonMode::Sensitive,
    }
}
