use super::{
    MongoFilter, MongoFindOptions, MongoSort, MongoSortDirection, MongoStore, codec,
    query::execute,
};
use crate::{
...
pub(super) async fn run(
    ...
    let sql = store.begin().await?;
    let transaction = Arc::new(MongoHookTransaction {
        store: store.clone(),
...
struct MongoHookTransaction {
    store: MongoStore,
    sql: Mutex<Option<crate::mongodb::query::MongoTransaction>>,
    active: AtomicBool,
}

impl MongoHookTransaction {
...
#[async_trait]
impl DatabaseTransaction for MongoHookTransaction {
    async fn find_by_id(
        &self,
        model: DatabaseModel,
        id: &str,
