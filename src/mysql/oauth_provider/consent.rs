...
use crate::{
    ...
    mysql::{
        MySqlFilter, MySqlFindOptions, MySqlSort, MySqlSortDirection, MySqlStore,
        query::execute,
    },
};
...
#[async_trait]
impl OAuthProviderConsentStore for MySqlStore {
    ...
    async fn find_oauth_consent_for_grant(
        ...
        find(
            ...
            &[
                ...
                eq("userId", user_id),
                MySqlFilter::equal("referenceId", json!(reference_id)),
            ],
        )
        .await
    }
    ...
    async fn upsert_oauth_consent(
        ...
        let filters = [
            ...
            MySqlFilter::equal("userId", json!(consent.user_id)),
            MySqlFilter::equal("referenceId", json!(consent.reference_id)),
        ];
            ...
            execute::find_one(&mut transaction, schema, "oauthConsent", &filters, &[]).await?
        {
            let existing_id = existing
                ...
                .ok_or_else(|| AuthError::Storage("invalid MySQL oauthConsent row: id".into()))?;
