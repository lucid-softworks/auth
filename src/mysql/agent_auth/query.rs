...
use crate::{
    ...
    mysql::{MySqlFilter, MySqlFindOptions, MySqlSort, MySqlSortDirection, MySqlStore},
};
...
pub(super) async fn list<T: AgentRecordOrder>(
    ...
    let values = store
        .find_records(model, &[eq(field, value)], &MySqlFindOptions::default())
        .await?
        .into_iter()
        .map(decode)
...
pub(super) fn first_by_id() -> MySqlFindOptions {
    MySqlFindOptions {
        sort: Some(MySqlSort {
            field: "id".into(),
...
pub(super) fn eq(field: &str, value: &str) -> MySqlFilter {
