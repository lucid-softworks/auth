...
use crate::{
    ...
    mysql::{MySqlFindOptions, MySqlStore, query::execute},
};
use async_trait::async_trait;
use serde_json::{Map, json};

#[async_trait]
impl OrganizationMemberStore for MySqlStore {
...
async fn find(
    store: &MySqlStore,
    filters: &[super::super::MySqlFilter],
) -> Result<Option<OrganizationMember>, AuthError> {
    store
...
async fn list(
    ...
    store
        .find_records("member", filters, &MySqlFindOptions::default())
        .await?
        .into_iter()
        .map(|record| super::codec::decode("member", record))
        .collect()
