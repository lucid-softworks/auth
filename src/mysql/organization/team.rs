...
#[async_trait]
impl OrganizationTeamStore for MySqlStore {
    ...
    async fn list_user_teams(&self, user_id: &str) -> Result<Vec<OrganizationTeam>, AuthError> {
        ...
        for member in members {
            let id = member
                ...
                .ok_or_else(|| {
                    AuthError::Storage("invalid MySQL teamMember row: teamId".into())
                })?;
...
async fn find(store: &MySqlStore, id: &str) -> Result<Option<OrganizationTeam>, AuthError> {
    store
