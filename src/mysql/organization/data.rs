...
use crate::{
    ...
    mysql::{
        MySqlFindOptions, MySqlSort, MySqlSortDirection, MySqlStore, query::execute,
    },
};
...
#[async_trait]
impl OrganizationDataStore for MySqlStore {
    async fn raw_insert_organization(
        ...
        codec::decode_organization(self.insert_required_record("organization", record).await?)
    }
        ...
        if schema.has_model("team") && schema.has_model("teamMember") {
            ...
            for team in teams {
                let team_id = team
                    ...
                    .ok_or_else(|| AuthError::Storage("invalid MySQL team row: id".into()))?;
                execute::delete_many(
                    &mut transaction,
                    schema,
                    "teamMember",
    ...
    async fn list_organizations(&self, user_id: &str) -> Result<Vec<Organization>, AuthError> {
        ...
        for member in members {
            let organization_id = member
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    AuthError::Storage("invalid MySQL member row: organizationId".into())
                })?;
