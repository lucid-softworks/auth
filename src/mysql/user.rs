use super::{MySqlComparisonMode, MySqlFilter, MySqlStore, codec, oauth, passkey, session};
mod write;
use crate::{
...
impl AuthStore for MySqlStore {
    ...
    async fn find_user_by_email(&self, email: &str) -> Result<Option<AuthUser>, AuthError> {
        let mut filter = MySqlFilter::equal("email", json!(email));
    ...
    async fn update_user_profile(
        ...
        write::update_user(self, &[MySqlFilter::equal("id", json!(id))], values).await
    }
    ...
    async fn update_user_email(
        ...
        let mut email = MySqlFilter::equal("email", json!(expected_email));
        email.mode = MySqlComparisonMode::Insensitive;
        write::update_user(
            self,
            &[MySqlFilter::equal("id", json!(id)), email],
            Map::from_iter([
    ...
    async fn update_password_hash(&self, user_id: &str, hash: String) -> Result<(), AuthError> {
        ...
        oauth::update(
            self,
            &account,
            &[MySqlFilter::equal("id", json!(account.id))],
        )
        .await?
        .ok_or(AuthError::CredentialAccountNotFound)?;
...
pub(super) async fn find(
    ...
    find_filtered(store, &[MySqlFilter::equal(field, json!(value))]).await
}
