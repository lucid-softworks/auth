...
use super::super::{SqliteFilter, SqliteStore, codec, oauth};
...
pub(super) async fn create_dependent_user(
    ...
    let record = codec::create_record(store, "user", &user, &id)?;
    let record = super::super::query::execute::insert(&mut transaction, schema, "user", record)
        .await
        .map_err(user_insert_error)?;
...
async fn write_user(
    ...
    match value {
        DatabaseWrite::Create(value) => {
            ...
            user.email = user.email.to_lowercase();
            let record = codec::create_record(store, "user", &user, &id)?;
            let stored = super::super::query::execute::insert(transaction, schema, "user", record)
                .await
                .map_err(user_insert_error)?;
            Ok((
...
pub(super) fn require_field(
    model: &super::super::schema::SqliteModel<'_>,
    field: &str,
) -> Result<(), AuthError> {
