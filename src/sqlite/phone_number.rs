...
#[async_trait]
impl PhoneNumberStore for SqliteStore {
    ...
    async fn create_phone_number_user(
        ...
        match self.insert_record("user", record).await {
    ...
    async fn update_user_phone_number(
        ...
        match self
            .update_record("user", &[SqliteFilter::equal("id", json!(user_id))], values)
            .await
        {
...
fn require_fields(store: &SqliteStore) -> Result<(), AuthError> {
