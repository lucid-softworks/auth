use super::{SqliteComparisonMode, SqliteFilter, SqliteStore, codec, oauth, query::execute};
use crate::{
...
#[async_trait]
impl SiweStore for SqliteStore {
...
async fn create_identity(
    ...
    let outcome = if let Some(owner) = address_owner {
        ...
        let mut email = SqliteFilter::equal("email", json!(user.email));
        ...
        let user = codec::decode::<AuthUser>(
            "user",
            execute::insert(&mut transaction, schema, "user", record).await?,
        )?;
...
async fn insert_wallet(
    ...
    decode_wallet(execute::insert(transaction, schema, "walletAddress", values).await?)
}
...
async fn owner_tx(
    ...
    let user = execute::find_one(
        ...
        schema,
        "user",
        &[SqliteFilter::equal("id", json!(wallet.user_id))],
        &[],
    )
    .await?
...
fn wallet_filters(address: &str, chain_id: Option<f64>) -> Vec<SqliteFilter> {
    let mut filters = vec![SqliteFilter::equal("address", json!(address))];
    if let Some(chain_id) = chain_id {
        filters.push(SqliteFilter::equal("chainId", json!(chain_id)));
    }
    filters
}
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid SQLite walletAddress row: {field}"))
}
