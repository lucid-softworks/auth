...
use crate::{
    ...
    sqlite::{SqliteStore, query::execute, schema::SqliteSchema},
};
...
async fn insert_organization_and_owner(
    ...
    *organization = codec::decode_organization(
        execute::insert(transaction, schema, "organization", record).await?,
    )?;
...
async fn insert_default_team(
    ...
    *team = codec::decode(
        "team",
        execute::insert(transaction, schema, "team", team_record).await?,
    )?;
    ...
    *team_member = codec::decode(
        "teamMember",
        execute::insert(transaction, schema, "teamMember", member_record).await?,
    )?;
    Ok(())
}
