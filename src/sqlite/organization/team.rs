...
#[async_trait]
impl OrganizationTeamStore for SqliteStore {
    async fn create_team(
        ...
        *team = codec::decode(
            "team",
            execute::insert(&mut transaction, schema, "team", record).await?,
        )?;
    ...
    async fn list_user_teams(&self, user_id: &str) -> Result<Vec<OrganizationTeam>, AuthError> {
        ...
        for member in members {
            let id = member
                ...
                .ok_or_else(|| {
                    AuthError::Storage("invalid SQLite teamMember row: teamId".into())
                })?;
...
async fn find(store: &SqliteStore, id: &str) -> Result<Option<OrganizationTeam>, AuthError> {
    store
