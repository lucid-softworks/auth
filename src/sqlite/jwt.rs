use super::{SqliteFindOptions, SqliteSort, SqliteSortDirection, SqliteStore};
...
impl JwkStore for SqliteStore {
    ...
    async fn create_jwk(
        ...
        decode(self.insert_record("jwks", record).await?)
    }
}
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid SQLite jwks row: {field}"))
}
