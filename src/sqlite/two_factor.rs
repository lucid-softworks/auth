use super::{SqliteFilter, SqliteStore, query::execute};
...
#[async_trait]
impl TwoFactorStore for SqliteStore {
    ...
    async fn upsert_two_factor(
        ...
        let inserted = execute::insert(&mut transaction, schema, "twoFactor", values).await?;
        transaction.commit().await.map_err(storage)?;
        decode(inserted)
    }
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid SQLite twoFactor row: {field}"))
}
