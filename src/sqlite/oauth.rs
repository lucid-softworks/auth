use super::{SqliteFilter, SqliteFindOptions, SqliteSort, SqliteSortDirection, SqliteStore, codec};
use crate::{
...
#[async_trait]
impl OAuthAccountStore for SqliteStore {
    ...
    async fn compare_and_swap_oauth_tokens(
        ...
        let filters = [
            ...
            SqliteFilter::equal("refreshToken", json!(expected_refresh_token)),
            SqliteFilter::equal("updatedAt", json!(expected_updated_at)),
        ];
...
pub(super) async fn create_user(
    ...
    let user_record = super::query::execute::insert(&mut transaction, schema, "user", user_record)
        .await
        .map_err(user_error)?;
...
pub(super) async fn insert(
    ...
    codec::decode_oauth(store.insert_record("account", record).await?)
}
...
pub(super) async fn insert_transaction(
    ...
    let record = codec::oauth_create_record(store, &account, &id)?;
    codec::decode_oauth(
        super::query::execute::insert(transaction, schema, "account", record).await?,
    )
}
