...
pub(super) struct PhysicalColumn {
    pub(super) field: AdditionalField,
}
...
pub(super) struct SqliteModel<'a> {
    logical_name: &'a str,
...
impl SqliteSchema {
    pub(super) fn new(schema: &ResolvedAdapterSchema) -> Result<Self, AuthError> {
        ...
        for runtime_input in schema.catalog().tables().keys() {
            ...
            logical_models.insert(
                ...
                enumerated
                    .get(canonical)
                    .expect("resolved SQLite model is enumerated")
                    .clone(),
            );
        }
        Ok(Self {
    ...
    pub(super) fn model(&self, logical: &str) -> Result<SqliteModel<'_>, AuthError> {
        let (logical_name, logical_model) =
            self.logical_models.get_key_value(logical).ok_or_else(|| {
                AuthError::InvalidConfiguration(format!(
                    "SQLite schema has no logical model '{logical}'"
                ))
            })?;
        let physical = self
            .models
            .get(&logical_model.physical)
            .expect("every logical SQLite model has a physical model");
        Ok(SqliteModel {
            logical_name,
...
impl SqliteModel<'_> {
    ...
    pub(super) fn encode(
        ...
        logical: &str,
        value: Value,
    ) -> Result<super::value::SqliteValue, AuthError> {
        if logical == "id" {
    ...
    pub(super) fn decode_all(&self, row: &SqliteRow) -> Result<Map<String, Value>, AuthError> {
    ...
    fn unknown_field(&self, logical: &str) -> AuthError {
        AuthError::InvalidConfiguration(format!(
            "SQLite schema model '{}' has no logical field '{logical}'",
            self.logical_name
        ))
    }
}

pub(super) struct SqliteWrite {
    pub(super) quoted_column: String,
    pub(super) value: super::value::SqliteValue,
}
...
mod tests {
    ...
    fn quotes_hostile_identifiers_and_resolves_remaps() {
        ...
        .unwrap();
        let schema = SqliteSchema::new(&resolved).unwrap();
