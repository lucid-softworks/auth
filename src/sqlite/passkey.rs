...
pub(super) async fn save(
    ...
    match store.insert_record("passkey", record).await {
...
pub(super) async fn compare_and_swap(
    ...
    Ok(store
        .update_record(
            ...
            &[
                eq("id", &passkey.id),
                SqliteFilter::equal("counter", json!(expected_counter)),
            ],
            values,
        )
        .await?
...
pub(super) async fn delete_for_user(store: &SqliteStore, user_id: &str) -> Result<(), AuthError> {
    store
