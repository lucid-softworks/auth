...
{
    let writes = rows::organization_writes(model, organization, id)?;
    let mut query = crate::postgres::rows::insert_query(model, writes);
    let row = query
        .build()
        .fetch_one(executor)
        .await
        .map_err(storage_error)?;
    rows::decode_organization(model, &row)
}
...
pub(super) fn update_query(
    ...
    query.push(" WHERE \"id\" = ");
    model
        .encode("id", json!(organization.id))?
        .push_bind(&mut query);
    query
