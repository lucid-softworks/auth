use super::{MssqlComparisonMode, MssqlFilter, MssqlStore, codec, oauth, query::execute};
use crate::{
...
async fn create_identity(
    ...
    let mut transaction = store.begin().await.map_err(storage)?;
    ...
    let outcome = if let Some(owner) = address_owner {
        ...
        let mut email = MssqlFilter::equal("email", json!(user.email));
...
async fn owner_tx(
    ...
    let user = execute::find_one(
        ...
        schema,
        "user",
        &[MssqlFilter::equal("id", json!(wallet.user_id))],
        &[],
    )
    .await?
...
fn wallet_filters(address: &str, chain_id: Option<f64>) -> Vec<MssqlFilter> {
    let mut filters = vec![MssqlFilter::equal("address", json!(address))];
    if let Some(chain_id) = chain_id {
        filters.push(MssqlFilter::equal("chainId", json!(chain_id)));
    }
    filters
}
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid MSSQL walletAddress row: {field}"))
}
