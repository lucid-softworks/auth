...
use crate::{
    ...
    mssql::{
        MssqlFilter, MssqlFindOptions, MssqlStore, MssqlTransaction,
        adapter::MssqlClient, query::execute,
    },
};
use serde_json::{Map, Value, json};
use std::collections::HashMap;

pub(super) async fn begin_immediate(store: &MssqlStore) -> Result<MssqlTransaction, AuthError> {
    store.begin().await.map_err(storage)
}

pub(super) async fn commit(connection: MssqlTransaction) -> Result<(), AuthError> {
    connection.commit().await.map_err(storage)
}

pub(super) async fn rollback(connection: MssqlTransaction) {
...
async fn load<T>(
    ...
    .map(|row| {
        let id = row
            ...
            .ok_or_else(|| AuthError::Storage(format!("invalid MSSQL {model} row: id")))?
            .to_owned();
        Ok((id, decode(row)?))
    })
    .collect()
...
async fn update_changed<T: PartialEq>(
    ...
    for (id, value) in after {
        if before.get(id).is_some_and(|previous| previous != value) {
            ...
            .await?
            .ok_or_else(|| {
                AuthError::Storage(format!("MSSQL {model} row disappeared during update"))
            })?;
        }
    }
    Ok(())
