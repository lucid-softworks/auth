...
use crate::{
    ...
    mssql::{
        MssqlComparisonMode, MssqlFilter, MssqlFindOptions, MssqlSort, MssqlSortDirection,
        MssqlStore, query::execute,
    },
};
...
#[async_trait]
impl OrganizationInvitationStore for MssqlStore {
    ...
    async fn resend_invitation(
        ...
        email_filter.mode = MssqlComparisonMode::Insensitive;
        let filters = [
        ...
        let id = latest
            ...
            .ok_or_else(|| AuthError::Storage("invalid MSSQL invitation row: id".into()))?;
        self.update_record(
            "invitation",
            &[eq("id", id)],
...
fn pending_filters(organization_id: &str, email: Option<&str>) -> Vec<MssqlFilter> {
    let mut filters = vec![
        eq("organizationId", organization_id),
        eq("status", "pending"),
    ];
