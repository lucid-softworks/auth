...
use crate::{
    ...
    mssql::{
        MssqlFindOptions, MssqlSort, MssqlSortDirection, MssqlStore, query::execute,
    },
};
...
#[async_trait]
impl OrganizationDataStore for MssqlStore {
    ...
    async fn list_organizations(&self, user_id: &str) -> Result<Vec<Organization>, AuthError> {
        ...
        for member in members {
            let organization_id = member
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    AuthError::Storage("invalid MSSQL member row: organizationId".into())
                })?;
