use super::{MssqlFilter, MssqlFilterOperator, MssqlStore, query::execute};
use crate::{
...
#[async_trait]
impl SecurityStore for MssqlStore {
...
async fn consume(
    connection: &mut crate::mssql::MssqlTransaction,
    ...
    let mut expired = MssqlFilter::equal("lastRequest", json!(now_ms.saturating_sub(prune_ms)));
    expired.operator = MssqlFilterOperator::Lt;
    execute::delete_many(connection, schema, "rateLimit", &[expired]).await?;
    let current = execute::find_one_for_update(
        connection,
        schema,
        "rateLimit",
        &[MssqlFilter::equal("key", json!(key))],
        &[],
    )
    .await?;
...
async fn update(
    ...
    .await?
    .ok_or_else(|| AuthError::Storage("MSSQL rate-limit row disappeared".into()))?;
    Ok(())
}
...
fn integer(record: &Map<String, Value>, field: &str) -> Result<i64, AuthError> {
    record
        ...
        .ok_or_else(|| AuthError::Storage(format!("invalid MSSQL rateLimit row: {field}")))
}
