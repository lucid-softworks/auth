...
use super::super::{MssqlFilter, MssqlStore, codec, oauth};
    ...
    match value {
        ...
        DatabaseWrite::Update(mut user) => {
            ...
            let stored = super::super::query::execute::update_one(
                ...
                schema,
                "user",
                &[MssqlFilter::equal("id", json!(user.id))],
                values,
            )
            .await?
...
async fn credential_account(
    ...
    super::super::query::execute::find_one(
        ...
        &[
            ...
            MssqlFilter::equal("providerId", json!("credential")),
        ],
        &[],
    )
    .await?
...
pub(super) async fn promote(
    ...
    let schema = store.physical_schema()?;
    let mut transaction = store.begin().await.map_err(storage)?;
    let filter = [MssqlFilter::equal("userId", json!(id))];
...
pub(super) async fn set_password(
    ...
    if account.id.is_empty() {
        ...
        let id = account.id.clone();
        oauth::update(store, &account, &[MssqlFilter::equal("id", json!(id))])
            .await?
            .ok_or(AuthError::CredentialAccountNotFound)?;
    }
...
pub(super) fn require_field(
    model: &super::super::schema::MssqlModel<'_>,
    field: &str,
) -> Result<(), AuthError> {
