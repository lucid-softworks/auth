use super::{MssqlFilter, MssqlFilterOperator, MssqlFindOptions, MssqlStore, codec};
...
#[async_trait]
impl ApiKeyStore for MssqlStore {
    async fn create_api_key(&self, key: DatabaseCreate<ApiKey>) -> Result<ApiKey, AuthError> {
        ...
        codec::decode_api_key(self.insert_required_record("apikey", record).await?)
    }
    ...
    async fn list_api_keys(
        ...
        self.find_records("apikey", &filters, &MssqlFindOptions::default())
            .await?
            .into_iter()
    ...
    async fn delete_expired_api_keys(&self, now: DateTime<Utc>) -> Result<u64, AuthError> {
        let mut filter = MssqlFilter::equal("expiresAt", json!(now));
        filter.operator = MssqlFilterOperator::Lt;
...
async fn find(store: &MssqlStore, field: &str, value: &str) -> Result<Option<ApiKey>, AuthError> {
    store
