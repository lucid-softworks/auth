use super::{
    MssqlFilter, MssqlFindOptions, MssqlSort, MssqlSortDirection, MssqlStore, codec,
    query::execute,
};
use crate::{
...
pub(super) async fn run(
    ...
    let sql = if store.adapter_config().transaction {
        store.begin().await.map_err(storage)?
    } else {
        store.sequential_connection().await.map_err(storage)?
    };
    let transaction = Arc::new(MssqlHookTransaction {
        store: store.clone(),
...
struct MssqlHookTransaction {
    store: MssqlStore,
    sql: Mutex<Option<MssqlTransaction>>,
    active: AtomicBool,
}

impl MssqlHookTransaction {
...
#[async_trait]
impl DatabaseTransaction for MssqlHookTransaction {
    async fn find_by_id(
        &self,
        model: DatabaseModel,
        id: &str,
    ...
    async fn create(
        ...
        let model_name = Self::model_name(model)?;
        let record = execute::insert_required(sql, schema, model_name, record).await?;
    ...
    async fn create_record(
        ...
        let mut sql = self.sql.lock().await;
        let sql = sql.as_mut().ok_or_else(closed_transaction)?;
        execute::insert_required(sql, schema, model, data).await
    }
