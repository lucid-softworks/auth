use super::{MssqlFilter, MssqlStore, codec};
use crate::{
...
#[async_trait]
impl PhoneNumberStore for MssqlStore {
    ...
    async fn update_user_phone_number(
        ...
        match self
            .update_record("user", &[MssqlFilter::equal("id", json!(user_id))], values)
            .await
        {
...
fn require_fields(store: &MssqlStore) -> Result<(), AuthError> {
