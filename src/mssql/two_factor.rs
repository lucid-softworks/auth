use super::{MssqlFilter, MssqlStore, query::execute};
...
#[async_trait]
impl TwoFactorStore for MssqlStore {
...
async fn record_failure(
    ...
    let Some(updated) = execute::increment_one(
        &mut transaction,
        schema,
        "twoFactor",
        &[eq("userId", user_id)],
        Map::from_iter([("failedVerificationCount".into(), json!(1))]),
        Map::new(),
    )
    .await?
    else {
    ...
    let next = updated
        .get("failedVerificationCount")
        .and_then(Value::as_u64)
        .ok_or_else(|| invalid("failedVerificationCount"))?;
    if next >= u64::from(max_attempts) {
        execute::update_one(
            &mut transaction,
            schema,
            "twoFactor",
            &[eq("userId", user_id)],
            Map::from_iter([("lockedUntil".into(), json!(locked_until))]),
        )
        .await?;
    }
    transaction.commit().await.map_err(storage)?;
    Ok(next >= u64::from(max_attempts))
}
...
fn invalid(field: &str) -> AuthError {
    AuthError::Storage(format!("invalid MSSQL twoFactor row: {field}"))
}
