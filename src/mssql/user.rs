use super::{MssqlComparisonMode, MssqlFilter, MssqlStore, codec, oauth, passkey, session};
mod write;
use crate::{
...
impl AuthStore for MssqlStore {
    ...
    async fn dash_create_record(
        ...
        self.insert_required_record(model, data).await.map(Some)
    }
    ...
    async fn find_user_by_email(&self, email: &str) -> Result<Option<AuthUser>, AuthError> {
        let mut filter = MssqlFilter::equal("email", json!(email));
        filter.mode = MssqlComparisonMode::Insensitive;
    ...
    async fn update_user_profile(
        ...
        write::update_user(self, &[MssqlFilter::equal("id", json!(id))], values).await
    }
    ...
    async fn update_user_email(
        ...
        let mut email = MssqlFilter::equal("email", json!(expected_email));
        email.mode = MssqlComparisonMode::Insensitive;
        write::update_user(
            self,
            &[MssqlFilter::equal("id", json!(id)), email],
            Map::from_iter([
    ...
    async fn update_password_hash(&self, user_id: &str, hash: String) -> Result<(), AuthError> {
        ...
        oauth::update(
            self,
            &account,
            &[MssqlFilter::equal("id", json!(account.id))],
        )
        .await?
        .ok_or(AuthError::CredentialAccountNotFound)?;
...
pub(super) async fn find(
    ...
    find_filtered(store, &[MssqlFilter::equal(field, json!(value))]).await
}
